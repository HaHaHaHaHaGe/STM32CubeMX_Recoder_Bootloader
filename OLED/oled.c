...
{  
	...
	{  
		...
		OLED_WR_Byte (0x10,OLED_CMD);      //ÉèÖÃÏÔÊ¾Î»ÖÃ¡g                                                                                                                                                                                 ªÁÐ¸ßµØÖ·   
		for(n=0;n<128;n++)OLED_WR_Byte(0,OLED_DATA); 
		//for(n=0;n<128;n++)OLED_WR_Byte(0,OLED_CMD); 
	} //¸üÐÂÏÔÊ¾
}
void OLED_On(void)  
{  
...
// 	
// 	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ
...
//	
// 	GPIO_SetBits(GPIOB,GPIO_Pin_12|GPIO_Pin_13);	//PD3,PD6 Êä³ö¸ß
...
//	OLED_WR_Byte(0xAF,OLED_CMD);//--turn on oled panel
	...
	OLED_WR_Byte(0xB0,OLED_CMD); /*set page address*/
	OLED_WR_Byte(0x81,OLED_CMD); /*contract control*/
	OLED_WR_Byte(0x4D,OLED_CMD); /*128*/
	...
	OLED_WR_Byte(0xDB,OLED_CMD); /*set COM pins*/
	OLED_WR_Byte(0x3F,OLED_CMD);
	OLED_WR_Byte(0xdb,OLED_CMD); /*set vcomh*/
	OLED_WR_Byte(0x3F,OLED_CMD);
	OLED_WR_Byte(0x32,OLED_CMD); /*set charge pump enable*/
